...
namespace log4net.Appender
{
    ...
    public class MultipleProcessRollingFileAppender : FileAppender
    {
        #region Public Enums
        ...
        private MemoryMappedFile m_sharedMemory;
        ...
        /// </para>
        /// </remarks>
        public MultipleProcessRollingFileAppender()
        {
            this.m_lastActiveTime = DateTime.Now;
            m_idleThread = new Thread(FileIdle);
            m_idleThread.Start();
        }

        private void FileIdle()
        {
            while (true)
            {
                try
                ...
                {
                    ErrorHandler.Error("check file is no used!", ex);
                }
            }
        }
        ...
        /// </summary>
        ~MultipleProcessRollingFileAppender()
        {
#if !NETCF
                ...
                if (m_mutexForRolling != null)
                {
                    while (true)
                    {
                        if (!m_mutexForRolling.WaitOne(10))
                        {
                            if (!m_isClosed)
                            {
                                LogLog.Debug(declaringType, "closed file");
                                this.CloseFile();
                                m_isClosed = true;
                            }
                            Thread.Sleep(10);
                        ...
                        if (m_isClosed)
                        {
                            this.SafeOpenFile(this.m_baseFileName, true);
                        }
                        break;
                    }
                }
        ...
        override protected void OpenFile(string fileName, bool append)
        {
            lock (this)
            {
                ...
                LogLog.Debug(declaringType, "openfile:" + fileName);
                ...
                else
                {
                    if (LogLog.IsErrorEnabled)
                    {
                        ...
                        {
                            LogLog.Debug(declaringType, "RollingFileAppender: INTERNAL ERROR. Append is False but OutputFile [" + fileName + "] already exists.");
                        }
                    }
                }
                ...
                LogLog.Debug(declaringType, "open file");
        ...
        protected string GetNextOutputFileName(string fileName)
        {
            if (!m_staticLogFileName)
            {
                ...
                //fileName += ("[" + Process.GetCurrentProcess().Id + "]");
            }
            return fileName;
        }
        ...
        override public void ActivateOptions()
        {
            ...
            string memoryMapName = "map_" + this.m_baseFileName
                           ...
                           .Replace(":", "_")
                           .Replace("/", "_");
            m_sharedMemory = MemoryMappedFile.CreateOrOpen(memoryMapName, 100, MemoryMappedFileAccess.ReadWrite);
            // initialize the mutex that is used to lock rolling
            m_mutexForRolling = new Mutex(false, "rolling_" + m_baseFileName.Replace("\\", "_").Replace(":", "_").Replace("/", "_"));
#endif
        ...
        protected void RollFile(string fromFile, string toFile)
        {
            if (FileExists(fromFile))
            {
                ...
                {
                    LogLog.Debug(declaringType, "rolling file", moveEx);
        ...
        protected void RollOverSize()
        {
            ...
            var accessor = m_sharedMemory.CreateViewAccessor();
            accessor.Write(0, m_curSizeRollBackups);
